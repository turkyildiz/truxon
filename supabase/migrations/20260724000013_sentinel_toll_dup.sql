-- R9 #81/#82 sentinels: toll-without-a-load and same-day duplicate loads.
-- #81 catches a toll charged to a truck that was not on any dispatched load at
-- that moment (personal use, a mis-scanned plate, or a toll on the wrong
-- unit). #82 catches the same customer + same lane entered twice on one day —
-- the classic accidental double-entry. Both auto-resolve when the anomaly
-- clears (toll rematched / a duplicate removed).
-- Lineage: full body from 20260724000012 (previous head) + TWO new blocks.
-- This file is the new lineage head for future sentinel splices.
...
begin
  ...
  -- (R9 #77) A draft invoice more than 48h old never went out the door. The
  -- work is billed in the system but the customer has not been asked to pay —
  -- money stuck one click from moving. Auto-resolves when it is sent.
  insert into _findings
  select 'invoice_unsent', 'cash', 'warn',
         'Invoices drafted but never sent',
         (select count(*) from public.invoices where status = 'draft' and created_at < now() - interval '48 hours')
           ||' invoice(s) have sat in draft over 48h - '
           ||to_char((select coalesce(sum(total),0) from public.invoices where status = 'draft' and created_at < now() - interval '48 hours'), 'FM$999,999,999')
           ||' not yet asked for. Send them from the Invoices page.',
         '', null
   where exists (select 1 from public.invoices where status = 'draft' and created_at < now() - interval '48 hours');

  -- (R9 #78) A delivered load with its POD on file but no invoice after 72h -
  -- we have the proof of delivery, we just have not billed. The slowest dollar
  -- is the one you earned and never invoiced. Auto-resolves once invoiced.
  insert into _findings
  select 'pod_not_invoiced', 'cash', 'warn',
         'Delivered with POD but not invoiced',
         (select count(*) from public.loads l
           where l.status in ('delivered','completed') and l.invoice_id is null
             and l.delivery_time < now() - interval '72 hours'
             and exists (select 1 from public.documents d
                          where d.entity_type = 'load' and d.entity_id = l.id and d.doc_type = 'POD'))
           ||' delivered load(s) have a POD on file but no invoice after 72h. Bill them from Invoices - Unbilled.',
         '', null
   where exists (select 1 from public.loads l
                  where l.status in ('delivered','completed') and l.invoice_id is null
                    and l.delivery_time < now() - interval '72 hours'
                    and exists (select 1 from public.documents d
                                 where d.entity_type = 'load' and d.entity_id = l.id and d.doc_type = 'POD'));
