-- Review H-1 (2026-07-21 code review): approved detention on an already-billed
-- load was stranded — proposals were created regardless of billing state, the
-- office approved them, but create_invoice only invoices loads with
-- invoice_id null, so the approved money was unreachable. Verified $0 stranded
-- on prod today (accessorials table empty — detention just went live), so this
-- closes the trap before the first dollar falls in:
--   1. propose_detention_accessorials only proposes on loads not yet invoiced.
--   2. sentinel_scan gains a critical `stranded_accessorial` finding for the
--      remaining race (load billed between propose and approve), with the
--      void-&-re-bill remedy in the finding text.
...
begin
  ...
  select d.load_id, 'detention', d.stop_type, d.est_pay, d.detention_min,
   ...
   where d.est_pay > 0
     -- H-1: never propose onto a load that's already invoiced — create_invoice
     -- could never collect it. Detention found after billing must go through
     -- void-&-re-bill or a supplemental invoice, not sit stranded here.
     and exists (select 1 from public.loads l
                  where l.id = d.load_id and l.invoice_id is null)
