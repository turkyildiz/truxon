-- Sentinel v8 — slow-pay finding on OUTSTANDING balance (owner bug report: the
-- forecast/Sentinel treated QBO-mirror invoices as open at full total, but most
-- carry only small factoring-fee residuals in the books). The finding now uses
-- the open amount (qbo_balance for mirrors, total − payments for native rows),
-- skips fee residuals (≤$200 AND ≤10% of total), and shows the real doc number.
-- Full sentinel_scan reproduced from the current version (single-function
-- requirement for auto-resolve); the previously-fired residual findings
-- auto-resolve on the next scan.
