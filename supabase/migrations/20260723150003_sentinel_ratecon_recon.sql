-- R9 #105 sentinel: rate-con reconciliation mismatch. When a scanned rate
-- con's line items are on file and their sum disagrees with the booked rate
-- by more than $1, someone is about to invoice the wrong number.
-- Lineage: full sentinel_scan body copied from 20260723001001_positive_role_gates.sql
-- (the lineage head) with ONE new _findings block spliced in before the upsert.
