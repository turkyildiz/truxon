-- Sentinel v6 — predictive slow-pay warning. The Northstar leap from "this
-- invoice IS overdue" (ar_overdue, already here) to "this invoice WILL land
-- late": each broker's own pay history (customer_pay_profile) projects which
-- OPEN 'sent' invoices will blow past their due date, while there's still time
-- to nudge. Fires warn, per invoice, auto-resolves the moment the invoice is
-- paid (leaves 'sent'). We INLINE the prediction (not slow_pay_risk()) because
-- that RPC is role-gated and Sentinel runs as service_role from cron; only
-- customer_pay_profile() (no gate, SECURITY DEFINER) is safe to call here.
-- Full sentinel_scan reproduced (single-function requirement for auto-resolve).
...
begin
  ...
  select 'slow_pay:'||r.invoice_id, 'cash', 'warn',
         ...
         '$'||r.total||' — '||r.customer||' averages '||round(r.avg_days)||' days to pay; predicts ~'
           ||r.predicted_days_late||' days past the '||to_char(r.due_date,'Mon DD')||' due date. Nudge now to protect cash flow.',
         'customer', r.customer_id
    from (
      select i.id as invoice_id, i.invoice_number, c.company_name as customer, i.customer_id, i.total,
