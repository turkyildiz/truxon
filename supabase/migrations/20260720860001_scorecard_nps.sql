-- R4 exam fix — the regression exam caught Forest saying driver NPS "needs
-- to be instrumented": the scorecard's not_captured list still named it after
-- the instrument shipped (migration 770001). not_captured is now down to
-- harsh-braking only, and the latest NPS quarter (null until responses land)
-- rides in the scorecard. Whole function reproduced from 20260720570001.
...
begin
  ...
  return jsonb_build_object(
    ...
    'driver_nps', (select to_jsonb(nps_row) from public.driver_nps_summary() nps_row
