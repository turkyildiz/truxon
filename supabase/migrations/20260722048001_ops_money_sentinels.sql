-- R9 #77/78/80/81/82/84/88: ops+money sentinel batch. Full sentinel_scan
-- redefinition (latest = 20260722044001, budget variance).
...
begin
  ...
  select 'fuel_darkday:'||f.truck_id||':'||f.d, 'money', 'warn',
    ...
    join public.trucks t on t.id = f.truck_id
   where not exists (select 1 from public.eld_daily_miles em
                      where em.truck_id = f.truck_id and em.day = f.d and em.miles > 1)
     and exists (select 1 from public.eld_daily_miles em2
                  where em2.truck_id = f.truck_id and em2.day > current_date - 7
                    and em2.miles > 5);
  ...
  select 'toll_notruck:'||x.truck_id||':'||x.d, 'money', 'warn',
   ...
   where not exists (select 1 from public.eld_daily_miles em
     ...
     and exists (select 1 from public.eld_daily_miles em2
                  where em2.truck_id = x.truck_id and em2.day > current_date - 7
                    and em2.miles > 5);
