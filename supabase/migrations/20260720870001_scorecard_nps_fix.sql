-- HOTFIX for 860001 — its driver_nps subquery aliased the summary as `s`,
-- colliding with the function's declared `record s` → "column reference s is
-- ambiguous" broke company_scorecard (caught by the local suite AFTER the
-- push; ordering mistake logged). Same function with alias nps_row.
