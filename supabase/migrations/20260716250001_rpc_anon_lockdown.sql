-- CRITICAL security fix (stress test, 2026-07-16). Every SECURITY DEFINER RPC
-- guards with `my_role() not in (...)`. For an ANONYMOUS caller my_role() is
-- NULL, and `NULL not in (...)` evaluates to NULL — not TRUE — so the guard
-- FAILS OPEN: the exception never fires and the function runs with definer
-- privileges (RLS bypassed). This let unauthenticated callers read
-- dashboard_summary / global_search / weekly_report (live financials, driver
-- names, customer roster, driver pay) and could have let them call the load/
-- invoice mutators.
--
-- Two layers of fix:
--   1. REVOKE EXECUTE from anon (and PUBLIC); GRANT only to authenticated —
--      an unauthenticated client can no longer call these at all.
--   2. Rewrite every guard as `coalesce(my_role(),'') not in (...)` so a NULL
--      role is treated as unauthorized (defense in depth for any authenticated
--      caller that somehow lacks a profile row).

-- ---------- layer 1: lock execution to authenticated ----------

do $$
declare
  fn text;
  fns text[] := array[
    'public.change_load_status(bigint, public.load_status)',
    'public.create_invoice(bigint, bigint[], timestamptz)',
    'public.void_invoice(bigint)',
    'public.set_invoice_status(bigint, public.invoice_status)',
    'public.weekly_report(date)',
    'public.dashboard_summary()',
    'public.global_search(text)',
    'public.check_rate_limit(text, integer, interval)',
    'public.next_load_number()',
...
declare
  ...
  result jsonb;
begin
  if coalesce(public.my_role(), '') not in ('admin', 'dispatcher', 'accountant') then
...
declare
  ...
  result jsonb;
begin
  if coalesce(public.my_role(), '') not in ('admin', 'accountant', 'dispatcher') then
    raise exception 'Not enough permissions';
  end if;
...
-- Re-grant after CREATE OR REPLACE (ownership grants persist, but the
-- revoke-from-public above already ran; make the authenticated grant explicit).
