-- Idle time, derived from the breadcrumbs we already have. DriveHOS exposes
-- no engine-hours (probed the roster raw payload — vin/number/active only),
-- but the breadcrumb stream itself is the engine-on signal: only ~1.7% of a
-- 7-day prod sample is STATIONARY, which rules out devices streaming while
-- parked. So: time-weight consecutive breadcrumbs per truck (gaps > 15 min =
-- engine off, excluded), attribute each interval to its leading status, and
-- idle %% = stationary time / engine-on time. Idle gallons uses the standard
-- 0.8 gal/hr idle-burn estimate, stated in the source. Flips #260, #261.

create or replace function public.idle_summary(p_days int default 30)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  with pts as (
    select truck_id, ts, status,
           extract(epoch from (lead(ts) over (partition by truck_id order by ts) - ts)) gap_s
    from eld_location_history
    where truck_id is not null and ts > now() - make_interval(days => p_days)
  ),
  ivals as (
    select truck_id,
           sum(gap_s) filter (where status = 'STATIONARY') stat_s,
           sum(gap_s) total_s
    from pts
    where gap_s is not null and gap_s between 1 and 900   -- >15 min gap = engine off
    group by truck_id
  ),
  fleet as (
    select coalesce(sum(stat_s), 0) stat_s, coalesce(sum(total_s), 0) total_s from ivals
  )
  select jsonb_build_object(
    'window_days', p_days,
    'engine_on_hours', round(f.total_s / 3600.0, 1),
    'idle_hours', round(f.stat_s / 3600.0, 1),
    'idle_pct', case when f.total_s > 0 then round(f.stat_s / f.total_s * 100, 1) end,
    'idle_gallons_est', round(f.stat_s / 3600.0 * 0.8, 1),
    'trucks', (select coalesce(jsonb_agg(jsonb_build_object(
                 'truck_id', i.truck_id,
                 'idle_pct', case when i.total_s > 0 then round(i.stat_s / i.total_s * 100, 1) end,
                 'idle_hours', round(coalesce(i.stat_s, 0) / 3600.0, 1))
                 order by coalesce(i.stat_s, 0) / greatest(i.total_s, 1) desc), '[]'::jsonb)
               from ivals i)
  ) into v from fleet f;
  return v;
end;
$fn$;
revoke all on function public.idle_summary(int) from public, anon;
grant execute on function public.idle_summary(int) to authenticated, service_role;

update public.playbook_metrics set status = 'live', updated_at = now(),
  source = 'idle_summary() — ELD breadcrumb time-weighting (STATIONARY vs IN_MOTION, 15-min gap cap = engine off); gallons via 0.8 gal/hr estimate'
where number in (260, 261) and status <> 'live';

-- scorecard: telematics gains idle_pct_30d; not_captured now names only harsh-braking
...
begin
  ...
  return jsonb_build_object(
    ...
    'not_captured', jsonb_build_array(
      'telematics harsh-braking events', 'driver turnover/NPS')
  );
end;
$$;
