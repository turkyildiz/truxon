-- Anchor fix: every factored_at is the 2026-07-19 Denim backfill date, so
-- 'aged since factoring' can never fire on the real backlog. Age from the
-- INVOICE date instead (90+ day-old invoice with only fee residue = crusty).
-- Lineage: supersedes 038002.
