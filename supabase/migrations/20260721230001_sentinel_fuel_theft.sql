-- Fuel-theft / card-misuse detection (2026-07-21).
-- Adds 5 fuel checks to sentinel_scan (product mismatch, cash advances,
-- tank overflow, rapid re-fuel, and a Tier-2 fuel-vs-miles reconciliation
-- using LOADED+DEADHEAD miles) plus an on-demand fuel_efficiency_by_truck RPC.
-- The sentinel_scan body is the prior version (sentinel_trends) with the
-- fuel blocks spliced in before the upsert — no existing check changed.

-- On-demand per-truck fuel picture: miles = LOADED (dispatch) + EMPTY (deadhead)
-- reconciled against gallons purchased, plus the theft-signal columns. Lets
-- Forest/the owner answer "which truck's fuel doesn't match its miles" directly.
create or replace function public.fuel_efficiency_by_truck(p_days int default 45)
returns table (
...
begin
  ...
  select t.id, t.unit_number,
         coalesce(mi.loaded,0), coalesce(mi.deadhead,0),
         coalesce(mi.loaded,0)+coalesce(mi.deadhead,0),
         coalesce(fu.gal,0),
         case when coalesce(fu.gal,0) > 0
              then round((coalesce(mi.loaded,0)+coalesce(mi.deadhead,0))/fu.gal, 2) end,
         round((coalesce(mi.loaded,0)+coalesce(mi.deadhead,0))/6.5),
         case when (coalesce(mi.loaded,0)+coalesce(mi.deadhead,0)) > 0
              then round((coalesce(fu.gal,0)/nullif((coalesce(mi.loaded,0)+coalesce(mi.deadhead,0))/6.5,0)-1)*100) end,
         coalesce(fu.diesel_spend,0), coalesce(fu.nonfuel,0), coalesce(fu.nondiesel_gal,0)
    from public.trucks t
    left join mi on mi.truck_id = t.id
