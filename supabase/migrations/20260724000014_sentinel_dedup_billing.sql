-- R9 fix: the billing sentinels I added in 20260724000012 (invoice_unsent,
-- pod_not_invoiced) DUPLICATED the pre-existing stale_drafts / pod_uninvoiced
-- checks — the daily brief was double-reporting each. This rebuilds sentinel_
-- scan cleanly FROM the pre-billing head (20260723150009, which never had the
-- dups) and re-adds only the genuinely-new toll_orphan + duplicate_load
-- checks. Net: the two duplicates are gone; every real check stays exactly once.
-- New lineage head.
