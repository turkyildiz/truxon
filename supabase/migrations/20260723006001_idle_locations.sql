-- R9 #50: idle heat-map — WHERE the fleet idles, not just how much.
-- Stationary stretches (>=30 min under 1 mph between breadcrumbs) classified
-- as customer-dock (within ~0.75 mi of any load stop coordinate in the
-- window) or elsewhere; the elsewhere list names its spots from the ELD's own
-- reverse-geocode strings. Dock idle is detention's territory; elsewhere is
-- routing/discipline territory.
