-- Sentinel v9 — DATA-HYGIENE findings. Trux now tells the office when the
-- data itself is lying: loads stuck "in transit" a week past appointment,
-- a driver double-booked across active loads (the loads #2/#11 case surfaces
-- itself instead of me silently editing prod), and delivered loads with no POD
-- after 14 days. Reproduced from the CURRENT v8 body (single-function
-- requirement: auto-resolve compares the full findings set).

alter table public.trux_insights drop constraint if exists trux_insights_category_check;
alter table public.trux_insights add constraint trux_insights_category_check
  check (category in ('money', 'cash', 'ops', 'compliance', 'maintenance', 'data'));
