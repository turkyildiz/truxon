-- R8 Block 1 — real MPG from telematics. The ELD breadcrumb backfill (Jun 29 →
-- today) means eld_daily_miles now covers the whole fuel-history window, so MPG
-- can finally be actual-miles ÷ gallons instead of booked-load miles ÷ gallons
-- (booked miles miss deadhead, out-of-route, and yard moves — they overstate
-- nothing but understate distance, inflating MPG error both ways).
--
--  • truck_mpg(p_days): per-truck MPG + fleet rollup + weekly fleet trend
--  • company_scorecard: fleet_mpg prefers ELD actual miles (booked fallback),
--    and says which basis it used
--  • playbook #259 source text updated to the ELD basis
...
begin
  ...
  with em as (
    ...
    select truck_id,
     ...
     where status <> 'Declined' and truck_id is not null
       ...
       -- MPG from tractor diesel only — DEF ("Diesel Exhaust Fluid"), gas,
       -- and cash-advance "Other" rows would skew the ratio
    ...
    select t.id as truck_id, t.unit_number,
           ...
           -- honesty floor: under 30 gal the tank buffer dominates the ratio
           case when coalesce(fu.gal, 0) >= 30 and coalesce(em.mi, 0) > 0
                then round(em.mi / fu.gal, 2) end as mpg,
    ...
    select date_trunc('week', transaction_time)::date as week_start, sum(gallons) as gal
      from fuel_transactions
     where status <> 'Declined' and transaction_time >= v_start
       and fuel_type ilike '%diesel%' and fuel_type not ilike '%exhaust%'
     group by 1
  ), wk as (
  ...
  select jsonb_build_object(
    ...
    'fleet', (select jsonb_build_object(
        ...
        'mpg', case when coalesce(sum(gallons), 0) > 0
                    then round(sum(eld_miles) / sum(gallons), 2) end,
    ...
    'basis', 'ELD actual miles (eld_daily_miles) ÷ diesel gallons; per-truck MPG suppressed under 30 gal')
    into v;
  return v;
end;
$$;
revoke all on function public.truck_mpg(int) from public, anon;
grant execute on function public.truck_mpg(int) to authenticated, service_role;

-- ── scorecard: fleet_mpg on the ELD basis ────────────────────────────────────
...
declare
  ...
  eld_mi numeric;      -- actual telematics miles in window (R8 Block 1)
  v_sales jsonb;
begin
  ...
  -- actual ELD miles driven in the window (all driving: loaded + deadhead +
  -- out-of-route). When present this is the honest MPG denominator basis.
  select coalesce(sum(miles),0) into eld_mi
    from public.eld_daily_miles
   where day >= p_start::date and day < p_end::date;
  ...
  return jsonb_build_object(
    ...
    'operations', jsonb_build_object(
      ...
      'fleet_mpg', case when gal>0 and eld_mi>0 then round(eld_mi/gal,2)
                        when gal>0 then round(loaded_mi/gal,2) end,
      'fleet_mpg_basis', case when gal>0 and eld_mi>0 then 'ELD actual miles'
...
-- ── playbook source honesty ──────────────────────────────────────────────────
update public.playbook_metrics
   set source = 'company_scorecard.operations.fleet_mpg — ELD actual miles ÷ gallons (breadcrumb backfill to 2026-06-29); per-truck detail in truck_mpg()',
       updated_at = now()
 where number = 259;
