-- R9 #77/#78 sentinels: billing hygiene. Two ways money gets stuck AFTER the
-- work is done — a draft invoice that never went out, and a delivered load
-- with its POD on file that nobody invoiced. Both are cash sitting on the
-- floor; both auto-resolve the moment the invoice is sent / created.
-- Lineage: full body from 20260723150009 (previous head) + TWO new blocks.
