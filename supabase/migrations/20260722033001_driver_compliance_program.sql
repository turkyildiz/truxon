-- R9 #20/21/28: driver compliance program log. One table for the DQF events
-- an auditor asks about beyond the credentials themselves: annual MVR reviews
-- (49 CFR 391.25), drug/alcohol test log + pool enrollment (part 382), and
-- annual Clearinghouse queries (382.701(b)). Records only - the actual MVR
-- pull / Clearinghouse query stays a human action; this is where it gets
-- logged so the sentinel stops nagging.
-- Full sentinel_scan redefinition (latest = 20260722032001, annual inspection).
create table if not exists public.driver_compliance_events (
  id bigserial primary key,
  driver_id bigint not null references public.drivers(id) on delete cascade,
  kind text not null check (kind in ('mvr_review','drug_test','alcohol_test','clearinghouse_query')),
  occurred_on date not null default current_date,
  result text not null default 'ok' check (result in ('ok','follow_up','fail')),
  reviewer text,
  notes text,
  created_by uuid default auth.uid(),
  created_at timestamptz not null default now()
);
create index if not exists dce_driver_kind_idx
  on public.driver_compliance_events (driver_id, kind, occurred_on desc);
alter table public.driver_compliance_events enable row level security;
-- Strip Supabase's default anon/authenticated ALL grant (posture tripwire)
-- down to what the office UI actually needs, and bless it.
revoke all on public.driver_compliance_events from anon, authenticated;
grant select, insert, delete on public.driver_compliance_events to authenticated;
drop policy if exists dce_select on public.driver_compliance_events;
create policy dce_select on public.driver_compliance_events
  for select to authenticated using (public.my_role() in ('admin','accountant'));
drop policy if exists dce_insert on public.driver_compliance_events;
create policy dce_insert on public.driver_compliance_events
  for insert to authenticated with check (public.my_role() in ('admin','accountant'));
drop policy if exists dce_delete on public.driver_compliance_events;
create policy dce_delete on public.driver_compliance_events
  for delete to authenticated using (public.my_role() = 'admin');
insert into app_private.security_baseline (kind, item) values
  ('grant', 'authenticated driver_compliance_events SELECT'),
  ('grant', 'authenticated driver_compliance_events INSERT'),
  ('grant', 'authenticated driver_compliance_events DELETE')
on conflict do nothing;

-- Pool enrollment lives on the driver row (it's a state, not an event).
alter table public.drivers
  add column if not exists drug_consortium text,
  add column if not exists drug_pool_enrolled_on date;
