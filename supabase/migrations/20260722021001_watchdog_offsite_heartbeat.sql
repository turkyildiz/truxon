-- Offsite NAS (INDIANCREEK) replication monitoring. The nightly backup.sh on
-- aida-nas posts {heartbeat:'offsite'} after a successful rsync of the
-- encrypted set to the second site. Full watchdog_db_probes redefinition
-- (latest = 20260722013004, KEEPING sentinel_stale) adding offsite freshness.
-- offsite_stale is NULL until the first successful mirror (no pre-setup alarm);
-- after that, >26h without a mirror means the offsite copy is drifting.
...
as $$
  select jsonb_build_object(
    ...
    -- Offsite NAS mirror freshness - NULL until the first successful rsync.
    'offsite_stale', (
      select (last_seen < now() - make_interval(hours => p_backup_stale_hours))
        from public.watchdog_heartbeats where source = 'offsite'),
    'offsite_last_seen', (select last_seen from public.watchdog_heartbeats where source = 'offsite'),
