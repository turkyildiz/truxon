-- R8: DOT audit-prep pack. One call = where we'd bleed in a compliance
-- review, with the honest not-tracked gaps named instead of glossed.
      ...
      select count(distinct t.id) from trucks t
       where t.status <> 'retired' and exists (
         select 1 from maintenance_records m
          where m.truck_id = t.id and coalesce(m.status,'') <> 'draft'
