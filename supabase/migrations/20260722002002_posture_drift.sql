-- ============================================================================
-- Permission-posture drift detector.
-- Three of Truxon's security reports independently found the same bug class:
-- a table quietly readable by `anon`, or RLS left off, or a SECURITY DEFINER
-- function missing its role gate. This makes that check continuous and
-- automatic. We snapshot a BASELINE of the current (blessed) posture, and the
-- sentinel fires whenever the live posture gains anything the baseline didn't
-- have: a new anon/authenticated grant, a newly anon-executable function, or a
-- sensitive table that lost RLS. Benign, intentional changes are absorbed by
-- re-blessing (bless_security_baseline, admin-only) — which is itself audited,
-- so you can't silently widen access and clear the alarm in one move.
-- ============================================================================

create table if not exists app_private.security_baseline (
  kind text not null,          -- 'grant' | 'routine' | 'rls_off'
  item text not null,          -- canonical identity string
  primary key (kind, item)
);

-- The live posture as a set of (kind, item) rows.
create or replace function app_private.security_posture()
returns table(kind text, item text)
language sql stable security definer set search_path = public, app_private
as $$
  -- anon/authenticated privileges on public base tables (views excluded — they
  -- run with definer/owner rights and are covered by their function gates)
  select 'grant',
  ...
  union
  -- functions anon can execute (a definer function anon can call is a door)
  select 'routine', r.grantee || ' ' || r.routine_name
  ...
  select 'rls_off', c.relname
  from pg_class c join pg_namespace n on n.oid = c.relnamespace
  where n.nspname = 'public' and c.relkind = 'r' and not c.relrowsecurity;
$$;
revoke all on function app_private.security_posture() from public, anon, authenticated;

-- Re-bless: accept the current posture as the new known-good. Admin-only and
-- audited (with a diff summary) so widening access always leaves a fingerprint.
...
$$;
revoke all on function public.bless_security_baseline() from public, anon;
grant execute on function public.bless_security_baseline() to authenticated, service_role;

-- Seed the baseline from the current (post-migration) state — this IS the
-- known-good, captured at deploy time.
insert into app_private.security_baseline
select kind, item from app_private.security_posture()
on conflict do nothing;

-- ---- sentinel check spliced below (full sentinel_scan redefinition) --------

-- ---- sentinel_scan: posture-drift check spliced in ----
