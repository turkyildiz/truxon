-- R9 #80/#85 sentinels: fuel-with-no-load and storage-growth anomaly.
-- #80 fuel_orphan mirrors toll_orphan for the fuel card — a fuel charge on a
-- truck that had no dispatched load at that time (the fuel-vs-miles check
-- catches over-fueling; this catches fuel on a truck that never rolled).
-- #85 storage_growth flags a month whose document uploads blew past the
-- trailing average — a runaway upload, a misconfigured import, or a leak.
-- Lineage: full body from 20260724000014 (previous head) + TWO new blocks.
-- New lineage head.
