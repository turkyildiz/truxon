-- Credit-exposure breach sentinel (R7 block 4): fleet-wide float guard +
-- a sentinel finding per over-limit broker. Full sentinel_scan redefinition.
-- Fleet-wide credit exposure (R7 block 4): the set-based sibling of
-- customer_exposure(id). Same rule — exposure (open AR + unbilled + committed
-- open loads) vs a limit of 1.5x avg monthly billed (6m), min $5k, halved when
-- the broker's avg pay exceeds 90 days. Returns only customers OVER their limit,
-- worst overage first. Reused by the sentinel + Forest.
create or replace function public.customers_over_exposure()
returns table (
  customer_id bigint, company_name text,
  exposure numeric, credit_limit numeric, over_by numeric, avg_days_to_pay numeric
)
...
begin
  ...
  with pay as (
    select i.customer_id,
           avg(extract(epoch from (i.paid_at - i.invoice_date)) / 86400.0) as days
      from public.invoices i
     where i.status = 'paid' and i.paid_at is not null
       and i.invoice_date > now() - interval '365 days'
     group by i.customer_id
  ), monthly as (
    select i.customer_id, sum(i.total) / 6.0 as m
      from public.invoices i
     where i.status in ('sent','paid') and i.invoice_date >= now() - interval '6 months'
     group by i.customer_id
  ), ar as (
    select i.customer_id, sum(public.invoice_balance(i)) as v
      from public.invoices i
     where i.status = 'sent' and i.factored_at is null
     group by i.customer_id
  ), unb as (
    select l.customer_id, sum(l.rate) as v from public.loads l
     where l.status = 'completed' and l.invoice_id is null group by l.customer_id
  ), openl as (
    select l.customer_id, sum(l.rate) as v from public.loads l
     where l.status in ('pending','assigned','in_transit') group by l.customer_id
  ), calc as (
    select c.id, c.company_name,
           coalesce(ar.v,0) + coalesce(unb.v,0) + coalesce(openl.v,0) as exposure,
           case when coalesce(pay.days,0) > 90
                then greatest(1.5 * coalesce(monthly.m,0), 5000) / 2
                else greatest(1.5 * coalesce(monthly.m,0), 5000) end as lim,
           pay.days
      from public.customers c
      left join ar on ar.customer_id = c.id
      left join unb on unb.customer_id = c.id
      left join openl on openl.customer_id = c.id
      left join monthly on monthly.customer_id = c.id
      left join pay on pay.customer_id = c.id
  )
  select calc.id, calc.company_name, round(calc.exposure), round(calc.lim), round(calc.exposure - calc.lim),
         round(calc.days::numeric, 1)
    from calc
   where calc.exposure > calc.lim and calc.exposure > 0
   order by (calc.exposure - calc.lim) desc;
end;
$$;
revoke all on function public.customers_over_exposure() from public, anon;
grant execute on function public.customers_over_exposure() to authenticated, service_role;
