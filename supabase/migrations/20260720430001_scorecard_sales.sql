-- Northstar night: fold the sales pipeline into the C-suite scorecard so Trux's
-- answers carry win-rate/pipeline and stop calling bids 'not captured'. Adds a
-- 'sales' section (from sales_pipeline) and removes bids from not_captured.
-- Reproduces the current company_scorecard body verbatim otherwise.
...
begin
  ...
  return jsonb_build_object(
    ...
    'not_captured', jsonb_build_array(
      ...
      'insurance loss ratio', 'DSCR/leverage')
  );
end;
$$;
