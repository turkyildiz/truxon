-- trux_query: read-only analytical SQL for the Trux agent, executed with
-- SECURITY INVOKER so the CALLER's RLS decides visibility — the agent gains
-- nothing beyond what the signed-in user could already query via PostgREST.
-- The keyword guard exists to stop the MODEL from accidental writes (writes
-- must go through confirm-gated tools), not as a permission boundary: the
-- permission boundary is RLS.
