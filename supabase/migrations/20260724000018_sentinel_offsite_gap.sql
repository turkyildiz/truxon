-- READINESS fix: the offsite-mirror monitoring gap. On 2026-07-23 the offsite
-- INDIANCREEK rsync died silently for ~28h and NOTHING alarmed — the watchdog
-- offsite_fresh check is warn-level (lost in the noise) and treats a missing
-- heartbeat as OK. This adds a CRITICAL sentinel finding on the reliable
-- trux_insights path: if the backup system is live (a 'backup' heartbeat
-- exists) but the offsite mirror heartbeat is missing or >26h stale, the
-- second-site recovery leg is broken — the class of failure the whole DR plan
-- exists to prevent. Auto-resolves when a fresh offsite mirror lands.
-- Lineage: full body from 20260724000015 + ONE new block. New lineage head.
...
begin
  ...
  -- (R9 #105) Rate-con reconciliation: extracted line items on file whose sum
  -- disagrees with the booked rate — the invoice is about to carry the wrong
  -- number (typo at booking, or a missed/short-paid accessorial).
  insert into _findings
  select 'ratecon_recon:'||x.load_id, 'money', 'warn',
         'Rate-con math off by $'||abs(x.delta)||' - load '||x.load_number,
         'Load '||x.load_number||' is booked at $'||coalesce(x.rate,0)||' but its scanned rate con itemizes to $'
           ||x.extracted_total||' ('||x.n_items||' line items). Fix the booked rate or the line items before invoicing.',
         'load', x.load_id
    from (select l.id load_id, l.load_number, l.rate,
                 sum(li.amount) extracted_total, count(*) n_items,
                 round(sum(li.amount) - coalesce(l.rate,0), 2) delta
            from public.load_line_items li
            join public.loads l on l.id = li.load_id
           where l.created_at > now() - interval '90 days'
           group by l.id, l.load_number, l.rate
          having abs(sum(li.amount) - coalesce(l.rate,0)) > 1.00) x;

  -- (R9 #102) Misfiled document: the classifier's fresh read disagrees with
  -- the stored label. Auto-resolves when the office relabels the doc (stored
  -- label changes → join breaks) or a newer audit agrees.
  insert into _findings
  select 'doc_misfiled:'||a.document_id, 'ops', 'warn',
         'Doc may be misfiled - '||left(d.filename, 60),
         '"'||d.filename||'" is filed as '||a.stored_type||' but reads like '||a.model_type
           ||' ('||coalesce(nullif(a.model,''),'NAS 3B')||', '||to_char(a.audited_at,'MM/DD')
           ||'). Open the doc and fix the label if the model is right.',
         d.entity_type, d.entity_id
    from public.doc_label_audits a
    join public.documents d on d.id = a.document_id
   where a.model_type <> a.stored_type
     and a.model_type <> 'Other'             -- an unsure model disputes nothing
     and d.doc_type = a.stored_type          -- still filed the disputed way
     and a.audited_at > now() - interval '30 days';

  -- (R9 #109) Doc-index freshness: docs are waiting to be embedded but the
  -- hourly NAS indexer hasn't written anything for 26h+ — the cron is dead or
  -- locked out (the doc-rag CRON_SECRET incident took days to notice; this
  -- makes it hours). Auto-resolves the moment fresh embeddings land.
  insert into _findings
  select 'doc_index_stale', 'ops', 'warn',
         'Doc indexing stalled - search going stale',
         (select count(*) from public.documents d
           where not exists (select 1 from public.document_embeddings e where e.document_id = d.id))
           ||' document(s) are waiting to be indexed but the NAS index job last wrote '
           ||coalesce(to_char((select max(created_at) from public.document_embeddings),'MM/DD HH24:MI'),'never')
           ||'. Check truxon-scheduler + run-index.sh logs on the NAS (and the CRON_SECRET gate).',
         '', null
   where exists (select 1 from public.documents d
                  where d.uploaded_at < now() - interval '3 hours'
                    and not exists (select 1 from public.document_embeddings e where e.document_id = d.id))
     and coalesce((select max(created_at) from public.document_embeddings), '-infinity') < now() - interval '26 hours';


  -- (R9 #81) A toll charged in the last 30 days to a matched truck that had NO
  -- load in motion at that moment — the truck tolled while nothing was
  -- dispatched to it. Could be personal use, a mis-read plate, or a toll on
  -- the wrong unit; worth a look before it is silently absorbed as cost.
  insert into _findings
  select 'toll_orphan', 'money', 'warn',
         'Tolls with no load rolling',
         (select count(*) from public.toll_transactions f
           where f.truck_id is not null
             and coalesce(f.post_date_time, f.exit_date_time, f.entry_date_time) > now() - interval '30 days'
             and not exists (select 1 from public.loads l
                              where l.truck_id = f.truck_id
                                and coalesce(f.post_date_time, f.exit_date_time, f.entry_date_time)
                                    between coalesce(l.pickup_time, l.created_at) - interval '6 hours'
                                        and coalesce(l.delivery_time, l.created_at) + interval '6 hours'))
           ||' toll charge(s) in 30 days hit a truck with no dispatched load at that time. Review on the Tolls report - a toll on an idle truck is worth a question.',
         '', null
   where exists (select 1 from public.toll_transactions f
                  where f.truck_id is not null
                    and coalesce(f.post_date_time, f.exit_date_time, f.entry_date_time) > now() - interval '30 days'
                    and not exists (select 1 from public.loads l
                                     where l.truck_id = f.truck_id
  ...
  -- (R9 #82) The same customer + same pickup + same delivery entered more than
  -- once on the same calendar day — almost always an accidental double-entry
  -- (a rate con dropped twice, a double-click on Create Load). Flag it so a
  -- phantom load does not ride into dispatch, billing and the forecasts.
  insert into _findings
  select 'duplicate_load', 'ops', 'warn',
         'Possible duplicate loads entered today',
         (select count(*) from (
            select l.customer_id, l.pickup_address, l.delivery_address, l.created_at::date d
              from public.loads l
             where l.created_at > now() - interval '7 days' and l.status <> 'cancelled'
               and l.pickup_address <> '' and l.delivery_address <> ''
             group by l.customer_id, l.pickup_address, l.delivery_address, l.created_at::date
            having count(*) > 1) g)
           ||' customer+lane combination(s) were entered more than once on the same day in the last week. Check Loads for a phantom - cancel the duplicate before it bills.',
         '', null
  ...
  -- (R9 #80) A fuel charge in the last 30 days on a matched truck that had NO
  -- load in motion at that time. The fuel-vs-miles check catches over-fueling
  -- a working truck; this catches fuel bought for a truck that was not even
  -- dispatched - a card used off the clock, or on the wrong unit.
  insert into _findings
  select 'fuel_orphan', 'money', 'warn',
         'Fuel bought with no load rolling',
         (select count(*) from public.fuel_transactions f
           where f.truck_id is not null and f.status <> 'Declined'
             and f.transaction_time > now() - interval '30 days'
             and not exists (select 1 from public.loads l
                              where l.truck_id = f.truck_id
                                and f.transaction_time between coalesce(l.pickup_time, l.created_at) - interval '8 hours'
                                                           and coalesce(l.delivery_time, l.created_at) + interval '8 hours'))
           ||' fuel charge(s) in 30 days hit a truck with no dispatched load at that time. Review on the Fuel page - fuel on an idle truck is worth a question.',
         '', null
   where exists (select 1 from public.fuel_transactions f
                  where f.truck_id is not null and f.status <> 'Declined'
                    and f.transaction_time > now() - interval '30 days'
                    and not exists (select 1 from public.loads l
                                     where l.truck_id = f.truck_id
  ...
  -- (R9 #85) This month's document uploads (by bytes) blew past 3x the trailing
  -- 3-month average, on a base big enough to matter (25MB+). Could be a
  -- legitimate batch import, but it is also what a runaway loop or a leak looks
  -- like - worth a glance at the Storage card before the bucket balloons.
  insert into _findings
  select 'storage_growth', 'data', 'warn',
         'Document storage growing abnormally this month',
         'Uploads this month are '
           ||round((select coalesce(sum(size_bytes),0) from public.documents
                     where uploaded_at >= date_trunc('month', now())) / 1048576.0, 0)
           ||'MB vs a ~'
           ||round((select coalesce(sum(size_bytes),0) from public.documents
                     where uploaded_at >= date_trunc('month', now()) - interval '3 months'
                       and uploaded_at < date_trunc('month', now())) / 3.0 / 1048576.0, 0)
           ||'MB monthly average. Check the Storage card for a runaway import or leak.',
         '', null
   where (select coalesce(sum(size_bytes),0) from public.documents
           where uploaded_at >= date_trunc('month', now())) > 26214400   -- 25MB floor
     and (select coalesce(sum(size_bytes),0) from public.documents
           where uploaded_at >= date_trunc('month', now()))
       > 3 * greatest((select coalesce(sum(size_bytes),0) from public.documents
                        where uploaded_at >= date_trunc('month', now()) - interval '3 months'
                          and uploaded_at < date_trunc('month', now())) / 3.0, 1);


  -- (READINESS) Offsite mirror freshness — the 07-23 gap. aida-nas posts an
  -- 'offsite' heartbeat after each successful INDIANCREEK rsync. If the local
  -- backup system is proven live (a 'backup' heartbeat exists) but the offsite
  -- heartbeat is missing or older than 26h, the second-site copy has stopped —
  -- a broken recovery leg that the warn-level watchdog check let hide. Critical.
  -- Quiet on a fresh DB with no backup heartbeat at all (local/dev/test).
  insert into _findings
  select 'offsite_mirror_stale', 'compliance', 'critical',
         '🗄️ Offsite backup mirror has stopped',
         'The offsite (INDIANCREEK) mirror last succeeded '
           || coalesce(to_char((select last_seen from public.watchdog_heartbeats where source = 'offsite'), 'Mon DD HH24:MI'), 'NEVER')
           || ' while nightly backups ARE running (last '
           || coalesce(to_char((select last_seen from public.watchdog_heartbeats where source = 'backup'), 'Mon DD HH24:MI'), '?')
           || '). The second-site copy is drifting - the exact silent failure the DR plan guards against. '
           || 'Check the truxon-offsite-rsync container + the INDIANCREEK volume (it runs near-full).',
         'security', null
   where exists (select 1 from public.watchdog_heartbeats where source = 'backup'
                  and last_seen > now() - interval '48 hours')     -- backup system is live
     and coalesce((select last_seen from public.watchdog_heartbeats where source = 'offsite'),
                  'epoch'::timestamptz) < now() - interval '26 hours';  -- offsite missing/stale
