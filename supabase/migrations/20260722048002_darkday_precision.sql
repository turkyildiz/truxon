-- Hotfix: fuel/toll no-mileage-day sentinels fired 19 warns on prod that
-- were eld_daily_miles BANK GAPS (the job skips days), not parked trucks.
-- Now the bank must show movement on both adjacent days before accusing
-- anyone, and the freshest 2 days are excluded (not banked yet).
-- Lineage: supersedes 048001.
