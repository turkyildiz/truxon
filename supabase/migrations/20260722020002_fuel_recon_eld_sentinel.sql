-- R8 Block 35b - fuel_recon on ELD actual miles. Full sentinel_scan
-- redefinition (latest = 20260722019002 with the Tier-2 reconciliation now
-- reading fuel_efficiency_by_truck(), which prefers GPS-actual miles).
...
-- weekly customer authority sweep (Tuesdays 06:40, after Monday's own-DOT pull)
do $$ begin perform cron.unschedule('truxon-fmcsa-customers'); exception when others then null; end $$;
select cron.schedule('truxon-fmcsa-customers', '40 6 * * 2',
  $job$select app_private.cron_edge_call('fmcsa-watch', '{"mode":"customers"}'::jsonb)$job$);
