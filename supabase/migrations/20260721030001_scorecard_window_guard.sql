-- HOTFIX for 20260721020001, caught on the first live digest fire: the
-- GL-anchored margins returned WHOLE-MONTH totals for a one-week window
-- (net $99.9K on $53.8K weekly revenue). GL basis now applies only when the
-- window spans >= 28 days; shorter windows use the operational basis, which
-- is week-grained. Whole function reproduced with the guard.
