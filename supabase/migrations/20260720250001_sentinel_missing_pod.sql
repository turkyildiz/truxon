-- Sentinel v4 — Forest nudges on missing PODs. A delivered/billed load with no
-- proof of delivery attached is cash stuck (brokers won't pay without it). Emitted
-- as ONE summary finding, not one-per-load (there can be 100+), and it calls out
-- how many already have a matching file waiting in the PODs archive to attach.
-- Full sentinel_scan reproduced (every check must live in one function so the
-- auto-resolve step sees the complete finding set); only the CASH section changed.
