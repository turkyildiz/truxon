-- Driver turnover, finally measurable — FORWARD. drivers had status
-- active/inactive/terminated but no date, so departures could not be timed and
-- the metric stayed needs_data. This adds terminated_at, stamps it whenever a
-- driver leaves active status (clears if re-activated), and computes turnover
-- from stamps going forward. Past departures have no honest date, so history
-- starts today — stated in the metric source rather than backfilled with
-- guesses. First-90-day turnover works off hire_date + terminated_at.

alter table public.drivers add column if not exists terminated_at date;

create or replace function public.drivers_stamp_termination()
returns trigger
language plpgsql
as $$
begin
  if new.status in ('inactive', 'terminated') and old.status = 'active' then
    new.terminated_at := coalesce(new.terminated_at, current_date);
  elsif new.status = 'active' and old.status in ('inactive', 'terminated') then
    new.terminated_at := null;
  end if;
  return new;
end;
$$;
drop trigger if exists drivers_termination_stamp on public.drivers;
create trigger drivers_termination_stamp
  before update of status on public.drivers
  for each row execute function public.drivers_stamp_termination();

create or replace function public.driver_turnover(p_start timestamptz, p_end timestamptz)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  select count(*) filter (where status = 'active'),
         count(*) filter (where terminated_at >= p_start::date and terminated_at <= p_end::date),
         count(*) filter (where terminated_at >= p_start::date and terminated_at <= p_end::date
                            and hire_date is not null and terminated_at - hire_date <= 90)
  into v_active, v_terms, v_terms_90
  from drivers;

  return jsonb_build_object(
    'active_drivers', v_active,
    'terminations_period', v_terms,
    'first_90_day_terminations', v_terms_90,
    'turnover_annualized_pct', case when v_active > 0
      then round(v_terms / v_days * 365.0 / v_active * 100, 1) end,
    'tracking_since', '2026-07-20'
  );
end;
$$;
revoke all on function public.driver_turnover(timestamptz, timestamptz) from public, anon;
grant execute on function public.driver_turnover(timestamptz, timestamptz) to authenticated, service_role;

update public.playbook_metrics set status = 'live', updated_at = now(),
  source = 'driver_turnover() — terminated_at stamps, LIVE-FORWARD since 2026-07-20 (no honest historical dates to backfill)'
where number in (507, 510) and status <> 'live';

-- scorecard: people section; not_captured names only driver NPS
