-- Sentinel v7 — detention nudge. Detention (measured by ELD dwell vs free time,
-- see detention_events) is billable money the broker owes that routinely goes
-- uncollected. Surface it as a CASH finding so it reaches Boss instead of
-- waiting in a tab. detention_events is role-gated and Sentinel runs as
-- service_role, so we first let the cron call it (service_role bypass), then
-- reproduce the full sentinel_scan (single-function requirement) with the
-- finding spliced into the CASH section.

-- Allow the cron (service_role) to call detention_events, like the scan's gate.
...
language plpgsql security definer set search_path = public stable as $BODY$
begin
