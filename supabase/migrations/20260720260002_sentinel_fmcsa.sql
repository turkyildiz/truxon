-- Sentinel v5 — FMCSA safety-rating watch. A lost safety rating (Conditional/
-- Unsatisfactory) or authority pulled ('not allowed to operate') is existential
-- for a carrier, so it fires critical. The BASIC over-threshold nudges already
-- exist (csa_alert) and go live automatically now that fmcsa-watch feeds
-- safety_csa. Full sentinel_scan reproduced (single-function requirement for the
-- auto-resolve step); only a finding added in the SAFETY section.
