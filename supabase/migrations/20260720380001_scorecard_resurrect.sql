-- Northstar night: resurrect the playbook points that new data now captures.
-- Since company_scorecard was written, tonight/earlier we instrumented FMCSA
-- safety (CSA/OOS/rating), ELD telematics (GPS/HOS), and detention (ELD dwell) —
-- and ELD arrival times let us finally compute on-time delivery. Light all four
-- up in the one function Trux uses for C-suite answers, and shrink not_captured.
-- Reproduces company_scorecard verbatim + adds safety/detention/telematics
-- sections, operations.on_time_delivery_pct, and honest remaining gaps.
...
begin
  ...
  return jsonb_build_object(
    ...
    'not_captured', jsonb_build_array(
      'telematics harsh-braking & idle events', 'budget vs actual', 'driver turnover/NPS',
      'bids/win-rate/pipeline', 'insurance loss ratio', 'DSCR/leverage')
  );
end;
$$;
