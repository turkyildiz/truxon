...
PREFER these pre-verified report functions over hand-written SQL for standard figures (SELECT * FROM fn(...)): company_scorecard(start,end) [~40 Owner's-Playbook metrics grouped financial/operations/revenue/maintenance/systems, PLUS a not_captured list of gaps] · pnl_summary(start,end) · weekly_report(date) · fuel_efficiency(start,end) [MPG + $/mile per driver, worst first] · fuel_by_truck(start,end) · fuel_ifta_summary(start,end) · toll_by_truck(start,end) · toll_by_agency(start,end) · ar_aging() [outstanding by customer, 30/60/90] · acct_summary() [KPI strip: A/R total, past-due $, DSO(90d), avg days-to-pay, unbilled loads $, MTD billed/collected] · acct_aging() [per-customer receivable buckets current/1-30/31-60/61-90/90+] · acct_unbilled_loads() [completed loads never invoiced — revenue leak with days unbilled] · acct_revenue_monthly(months) [billed vs collected by month] · acct_revenue_by_customer(days) [broker revenue, share % concentration, open/past-due, avg days-to-pay] · acct_margin_monthly(months) [revenue vs fuel+tolls+MX direct costs] · gl_pnl_monthly(months) [TRUE P&L from the books: gross/net margin %, TRUE operating ratio with ALL costs] · gl_expense_breakdown(months) [every expense account, % of revenue] · gl_breakeven_monthly(months) [actual RPM vs break-even RPM from all costs + all miles] · gl_cfo_snapshot() [cash on hand, days of cash, current ratio, working capital, DPO, interest coverage, overhead/tractor, total cost of risk] · safety_summary(start,end) [accidents/million-mi, OOS rate, HOS, claims, csa_basics_in_alert; per-BASIC CSA percentiles live in table safety_csa — raw SQL it] · budget_variance(start,end) [budget vs actual per P&L line] · maintenance_summary(start,end) [planned vs reactive spend, PM-compliance %, deadlined-tractor %, open work orders, by-service, top cost units] · maintenance_cpm(start,end) [fleet Maintenance CPM & Tire CPM] · maintenance_by_truck(start,end) [cost + CPM per unit] · maintenance_by_vendor(start,end) [outsourced-shop spend] · maintenance_due() [per-unit PM/inspection status: miles/days remaining, due_status overdue|due_soon|ok|never_serviced|unknown] · maintenance_alerts() [what needs attention now: PM/inspection due + plate expiry + stale work orders] · playbook_coverage() [how many of the 1000 Owner's-Playbook metrics are live vs needs_data vs external, by category] · playbook_metrics_list(status,owner,search) [browse the metric catalog]. · cashflow_forecast(weeks) [4-8wk cash in/out/net: open AR by predicted BOOK pay date + booked uninvoiced + recurring costs. HONESTY: brokers here average ~100 days-to-pay on the books, so little lands inside 4 weeks; actual CASH arrives earlier via factoring advances, which the books don't time — say so when the forecast looks empty] · slow_pay_risk() [open invoices predicted to land >15d late from each broker's own pay history, with real OUTSTANDING amounts] · customer_pay_profile() [per-broker days-to-pay distribution, WITH customer name — always show names, never bare customer IDs] · revenue_forecast(weeks) [weekly revenue outlook, trailing avg + same-week-last-year] · customer_rate_profile(customer_id) [what this broker has paid $/mi, 180d] · fleet_cost_basis() [MPG, fuel price, pay/fixed/toll per mile, break-even RPM] · detention_events(days) [per-stop billable detention measured from ELD dwell at geocoded stops; each PROPOSED charge also banks its dispute-ready exhibit in load_accessorials.evidence (appointment, ELD arrival/departure, dwell math) which outlives the 2-day GPS retention — for "what proof do we have" cite that banked evidence] · stop_dwell_summary(days) [avg dwell hours at shipper vs consignee] · pod_capture_rate(start,end) [% of delivered loads with a POD within 12h — owner standard] · sales_pipeline(start,end) [quotes received/won/lost/open, win rate. The pipeline IS wired: quote-classified emails auto-promote into quote_requests every 2h when the extraction has a locatable lane — an empty pipeline means no lane-complete quote emails arrived, not a missing integration] · fleet_ops_extras(start,end) [deadhead/dispatch, miles per driver-week, loads & miles per day] · gl_balance_ratios() [debt/equity, net debt, net-debt/EBITDA, ROE off the balance-sheet mirror] · insurance_snapshot() [premiums 12m from GL, claims, LOSS RATIO, insurance CPM, open claims] · idle_summary(days) [fleet + per-truck idle % derived from ELD breadcrumbs, est. idle gallons] · driver_turnover(start,end) [terminations, first-90-day losses, annualized % — tracked since 2026-07-20] · weekly_flash(week_offset) [the weekly owner one-pager: ops/cash/safety/sentinel/budget on the Mon-Sun week standard] · metric_trends(prefix) [WoW/MoM change + 13-week slope for every nightly-snapshotted metric series] · segment_economics(start,end) [the money view BY SEGMENT: per-truck / per-driver / per-customer revenue, $/mi, weekly revenue, per-customer margin at the GL all-in cost per mile, plus fleet block: revenue per tractor/driver per week, EBITDA per tractor, % revenue below variable cost, multi-stop %, customer churn %, top-quartile margin gap] · lane_summary(days) [every state→state lane ranked: loads, revenue, $/mi, margin at GL all-in cost, deadhead %, below-breakeven flag] · driver_scorecard(week_offset) [weekly per-driver card: loads, miles, revenue, $/mi, pay, ELD on-time %, detention hours, violations] · customer_profile(customer_id) [one customer judged whole: 12m revenue/loads/$-mi, margin at GL all-in RPM, monthly trend, avg days-to-pay, open+past-due OUTSTANDING AR, open loads, unbilled, detention at their docks, docs on file] · lane_rate_history(origin_state, dest_state) [single-lane $/mi benchmark for booking] · stress_test() [CFO stress pack: baseline + revenue −25% / fuel +40% / insurance +30% / perfect storm, each with shocked monthly net + cash runway in months] · scenario_runway(revenue_pct, fuel_pct, insurance_pct) [custom what-if: e.g. (-10, 20, 0); fuel scales with volume AND price, other costs fixed — state the assumptions when answering] · collections_queue() [prioritized overdue call list: contact info, overdue $, oldest days, avg days-to-pay, latest promise-to-pay] · driver_nps_summary() [quarterly driver NPS from the in-app survey: responses, promoters/passives/detractors, NPS, comments WITHOUT names — keep them anonymous] · denim_reconciliation() [Denim's statement vs our books: jobs matched, fee totals both sides, per-invoice fee mismatches, Denim jobs with no invoice, invoices marked factored that Denim never saw] · factoring_cost_summary() [what factoring costs: effective rate % (true fees ÷ face), monthly fee buckets, days of float gained vs book pay speed, annualized cost % — for "is factoring worth it" use THIS] · qbo_writeoff_list() [fee-sliver write-off proposals + the approved accountant packet; approval never touches QBO — the office applies it by hand] · payment_application_audit() [payments that landed wrong: paid here but open in QBO, collected in QBO but open here, payments past invoice total] · credit_memo_summary(months) [credit memos from the QBO mirror: count, $, credit-memo rate %, invoice accuracy % (playbook #72/#73)] · rev_rec_drift(months) [revenue earned (delivery month) vs booked (invoice month) + cross-month loads; a persistent gap with zero cross-month loads = unlinked QBO-history invoices, not timing] · per_truck_pnl(months) [each unit's own ledger: revenue on its loads − its fuel/tolls/maintenance/driver pay/payment, net + ROI (ROI blank until payments entered on the truck forms)] · truck_utilization(days) [ELD-banked moving vs parked days per unit, weekend-miles %, revenue per moving day] · depreciation_schedule() [owner-view straight-line (60mo, 20% salvage) from purchase price/date on the truck forms — NOT the tax books] · facility_dwell_league(days) [dock-time league: dwell avg/P50/P90 + detention hours and $ per facility (customer+state+side) — the exhibit for rate conversations] · load_eta_risk() [rolling loads vs their delivery appointment: miles to go, ETA (straight-line ×1.25 @47mph net), HOS-checked risk late|hos_short|tight|ok — for "will we make it" use THIS] · speeding_trend() [minutes at 75+ per truck this week vs last with delta — coaching direction] · driver_compliance_events table + dot_audit_pack cover MVR/drug/alcohol/Clearinghouse logs [records only; the actual pull/query is human] · anomaly_digest(threshold_pct) ["what changed this week" as sentences from the nightly metric snapshots; states its own readiness while history is young] · fuel_stop_analysis(days) [per merchant: visits, gallons, price paid vs our own same-state average and the premium $ it cost — for "where should we fuel" use THIS; no market feed, behavior vs ourselves] · idle_locations(days) [WHERE the fleet idles: dock vs elsewhere hours + the named top elsewhere spots; overnight rest is expected] · cancellation_analytics(days) [who cancels, how often, revenue walked, per customer] · deadhead_patterns(days) [which delivery regions strand trucks: per-state repositioning miles + worst from→to pairs] · doc_retention_report(days) [file completeness: loads RC/POD/BOL %, driver license/med-card doc %, truck registration/insurance doc %] · driver_tenure_summary() [avg/median months + buckets from hire_date] · dispatch_productivity(days) [loads per dispatcher per working day] · sentinel_weekly_digest() [the whole open board grouped by category, snooze-aware] · dot_audit_pack() [the DOT audit binder in numbers: CDL + med-card on file / expired / expiring, annual inspections current (formal dot_inspection records), MVR / Clearinghouse / testing-pool coverage 12m, DQF-complete count, ELD reporting, DVIR adoption, plates, plus a not_tracked honesty list. For "are we ready for a DOT audit" use THIS] · dvir_summary(days) [driver vehicle inspections from the app: who inspected what and when, defect + not-safe flags; defects auto-file into maintenance (source dvir). "Did everyone do a pre-trip today?" lives here] · table weather_alerts [NWS severe-weather warnings pushed to trucks at their live positions — event, severity, headline, per truck; raw SQL it for "any storms on the fleet?"] · load_actuals(days) [per completed load: booked vs ACTUAL variable margin — real driver pay, transponder tolls, banked ELD miles (deadhead included) x GL fuel $/mi. HONESTY: variance is structurally $0 for loads delivered before the ELD miles bank began (2026-07-19) — it falls back to booked miles; say so instead of calling estimates "perfect"] · customer_keep_fire(days) [the quarterly keep-or-fire call for EVERY customer: margin at GL all-in cost, pay speed, detention burden, revenue share → grow/keep/fix-price/fire with the reason spelled out. Use THIS for fire/keep questions (report the full counts per call, then highlight the worst) — don't hand-roll the ranking] · customer_exposure(customer_id) [total float on one customer: open AR + unbilled + committed loads vs a pay-history-derived limit (1.5x monthly, min $5k, halved for >90d payers); over_limit flags credit risk before booking more] · ops_service_metrics(days) [on-time PICKUP %, on-time DELIVERY %, combined on-time SERVICE %, and missed pickup/delivery rates — measured from ELD arrival vs the appointment (+2h grace), only for ELD-covered legs. For "are we on time / service failures / did we miss appointments" use THIS; on_time_delivery_pct in company_scorecard is the same delivery number] · customer_revenue_extras(days) [ADMIN/ACCOUNTANT — unprofitable customer count, top-decile profit % of total, bottom-decile profit $, average customer relationship length in years. Contract/spot mix is NOT captured (no rate-type flag) — say so if asked] · customers_over_exposure() [every broker whose float (open AR + unbilled + committed open loads) is OVER its credit limit (1.5x avg monthly billed, $5k floor, halved for >90d payers), worst overage first — the fleet-wide version of the booking-screen guard. For "who's over their credit limit / who are we financing" use THIS] · security_scorecard() [ADMIN/ACCOUNTANT ONLY — the cyber-security posture in numbers: MFA coverage % of office users, cyber incidents 30d (critical audited events), open critical security findings, honeypot hits, data-reconciliation exceptions 7d, POD OCR success %, audit-chain-intact, ransomware-guard-armed. For "are we secure / any intrusions / who has MFA" use THIS; the full console is at /security] · finance_extras() [ADMIN/ACCOUNTANT ONLY — accessorial revenue 90d, detention capture rate % (share of decided detention proposals the office approved/billed), billing lag days (delivery → invoice), AR > 45/60/90 open balances. For "how fast do we bill / are we capturing detention / how old is our AR" use THIS] · truck_mpg(days) [REAL fleet + per-truck MPG: day-matched ELD GPS miles ÷ diesel gallons + weekly trend; the honest efficiency number — use THIS for MPG, not card-implied ratios] · fuel_efficiency_by_truck(days) [fuel-vs-miles DIVERSION reconciliation: gallons counted only on GPS-covered days, variance vs 6.5 MPG expected, gallons_untracked = fuel bought while the ELD was dark (its own red flag), miles_basis eld|booked. For "is anyone stealing fuel" use THIS] · speeding_summary(days) [per-driver/truck minutes at 75+ mph from ELD pings — coaching list, worst first] · eld_fleet_live() [right-now board: per truck last ELD ping, odometer, current driver, HOS drive-hours remaining — for "who has hours / where is everyone"] · finance_march() [ADMIN/ACCOUNTANT — YTD revenue growth YoY (GL basis), QTD EBITDA margin, freight revenue 12m, lease expense 12m, top-10 customer profit concentration, % of 90d revenue booked BELOW variable/full cost (the pricing leak), DSO/DPO/cash-conversion-cycle days] · load_route(load_id) [the GPS breadcrumb trail one load actually drove — exists for loads since the mileage bank (2026-06-29)] · budget list note: budgets auto-seed monthly from trailing-3-month actuals (basis='auto'); the office can overwrite any line manually. For broad "how's the business" or scorecard questions, call company_scorecard first. Use raw SQL only for questions these don't cover.
