...
- special_terms: short string (max 300 chars) with what a dispatcher must know: extra stops beyond the first pickup/last delivery, required equipment, temperature, tracking demands, unusual penalties. null if nothing notable.
...
Deno.serve(async (req) => {
  ...
  const prompt = extractionPrompt(settings?.company_name || 'the carrier')
