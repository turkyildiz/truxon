...
Deno.serve(async (req) => {
  ...
  if (req.method === 'POST') {
    ...
    if (link_driver_id != null && data.user) {
      const { error: linkErr } = await admin
        ...
        .eq('id', link_driver_id)
      if (linkErr) {
        // Best-effort rollback of the auth user so we don't leave orphan logins.
  ...
  if (req.method === 'PATCH') {
    ...
    // Prevent demoting/deactivating the last remaining admin.
    ...
    // Prevent self-lockout without a second admin (same last-admin rule covers it).
