...
**Live bank balance (2026-07-23, commit 86257d0):** `qbo-sync mode:bank_balances` (cron/admin gated, read-only) pulls Account `AccountType='Bank'` fresh; Forest tool `bank_balance` (admin+accountant only, rides the cron door via CRON_SECRET — DB copy key is `cron_secret` lowercase) answers "what's our bank balance" at ask time. NOTE: Huntington register runs NEGATIVE on the books (factoring advances/deposits unrecorded in QBO) — tool description teaches Forest to explain the artifact, not alarm. Verified end-to-end via the temp-admin exam harness.
