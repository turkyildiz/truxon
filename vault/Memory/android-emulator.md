...
3. Login driver2@aidalogistics.com. DENY location + notifications (keeps the fleet map clean — no fake GPS). adb typing: tap the field, `input keyevent 123` (cursor to end), delete-loop, `input text`, `keyevent 61` (tab) / `66` (enter); taps mid-flow miss when the keyboard shifts the layout.
