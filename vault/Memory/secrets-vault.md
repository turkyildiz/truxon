...
**POPULATED 2026-07-23 (the "empty vault" scare was real but harmless — it had simply never been populated before the machine swap).** Owner ran `~/dev-tools/secrets/attach-all.sh` (Claude-staged; prompts for master password once, pipes it only into keepassxc-cli): 12 file attachments now in the vault — `truxon-release.jks` + `key.properties` (Mobile-Signing), `google-services.json` (Integrations/firebase-fcm), all 7 NAS env files incl. backup.env/BACKUP_PASSPHRASE + b2.env (NAS/nas-env-files), dev-box SSH keypair (NAS/devbox-ssh-key). Pushed to NAS (11,406 bytes, version 20260723-161857); nightly B2 sweep makes it offsite+immutable. **Script kept** at `~/dev-tools/secrets/attach-all.sh` for future re-runs. **Still owner-manual:** service_role key, prod DB password, CRON_SECRET, ITS login (its.env is root-owned on NAS — unreadable to us), account logins + MFA recovery codes.
