...
**How to apply:** STANDING PERMISSION (owner, 2026-07-23): "you always have my permission to git push" — push commits without asking. (The auto-mode classifier still blocks pushes intermittently; when it does, hand Ilker the command.) For prod DB pushes / destructive ops the earlier rule stands: get a go-ahead, routine pre-go-live. Note: the harness classifier has intermittently blocked `supabase db push` — if blocked, hand Ilker the exact commands. Local dev: `supabase start` + `supabase db reset` is the documented local-only workflow (safe). Migrations are forward-only — never edit an applied one. Frontend uses generated types in `frontend/src/database.types.ts` — regenerate with `supabase gen types typescript --local` after schema changes. See [[user-ilker]].
