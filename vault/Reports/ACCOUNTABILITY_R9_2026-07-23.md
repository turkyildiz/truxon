...
- **#81/#82 toll-orphan + duplicate-load sentinels** (lineage head → `20260724000013`). **#81 toll_orphan** (money/warn): a toll charged in 30d to a matched truck that had NO dispatched load covering that moment — personal use, a mis-read plate, or the wrong unit; auto-resolves when a load covers the toll time. **#82 duplicate_load** (ops/warn): the same customer + same pickup + same delivery entered more than once on one calendar day (7d window) — the accidental double-entry, flagged before the phantom rides into dispatch/billing/forecasts; auto-resolves when it's cancelled. Suite: **1125 pgTAP / 192 files**, anon 42501 verified. `a3612ac`. Harness note: `loads.status` is trigger-guarded, so the dup-resolve test cancels via `cancel_load()`.

- **Self-caught + fixed: duplicate billing sentinels.** Scouting this lap I noticed `stale_drafts` and `pod_uninvoiced` already existed — meaning last lap's #77/#78 (`invoice_unsent`/`pod_not_invoiced`) were **functional duplicates** firing on the same conditions, so the daily brief would double-report each (the exact noise #86/#176 exist to prevent). Rebuilt `sentinel_scan` from the pre-billing head (`20260723150009`, no dups) and re-added only the genuinely-new `toll_orphan` + `duplicate_load`; every real check now appears exactly once (new head `20260724000014`). Test 186 repointed to the canonical keys and now asserts the dup keys produce zero findings. Two failed splice attempts along the way (`extract_block` grabbed the `select` but not its preceding `insert into _findings`) — fixed by anchoring extraction on the insert line. Suite: **1124 pgTAP / 192 files**, anon 42501 verified. `86ac45c`. **Lesson logged: always grep existing dedup_keys before adding a sentinel.**
